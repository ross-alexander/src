...
# songs_bot.py
#
# Original code from https://github.com/vrano714/py_discord_local_music
#
# 2021-11-03: Ross Alexander
#    Add --path
#
# ----------------------------------------------------------------------
...
class MusicBot(commands.Cog):
    def __init__(self, bot, db):
        ...
        self.parser.add_argument("--path", action="store", nargs="+")
    ...
    def parse(self, query):
        ...
        if "path" in play_args and play_args.path is not None:
            query_path = play_args.path[0]
            print("Query path: %s" % query_path)
            tmp = [x for x in self.song_database if query_path == x["filename"]]
            print(tmp)
            names = [x["song_keys"][0] for x in tmp]
            print(names)
#            print("Query path found: %" % " ".join(names))
