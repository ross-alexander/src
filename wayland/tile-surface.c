...
tile_surface_t* tile_surface_new_from_surface(cairo_surface_t *surface, int width, int height)
{
  tile_surface_t *tile = calloc(1, sizeof(tile_surface_t));
  assert(width > 0);
  assert(height > 0);
...
tile_surface_t* tile_surface_new(int width, int height)
{
  tile_surface_t *tile = calloc(1, sizeof(tile_surface_t));
  tile->width = width;
  tile->height = height;
...
tile_surface_t *tile_surface_new_from_file(const char *path)
{
  GError *error;
