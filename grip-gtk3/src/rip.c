...
void MakeRipPage(GripInfo *ginfo)
{
  ...
  //  gtk_tooltips_set_tip(MakeToolTip(),button, _("Rip and encode selected tracks"),NULL);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(DoRipEncode),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),button,FALSE,FALSE,0);
  gtk_widget_show(button);

  button=gtk_button_new_with_label(_("Rip Only"));
  //  gtk_tooltips_set_tip(MakeToolTip(),button, _("Rip but do not encode selected tracks"),NULL);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(DoRip),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),button,FALSE,FALSE,0);
  gtk_widget_show(button);

  button=gtk_button_new_with_label(_("Abort Rip and Encode"));
  //  gtk_tooltips_set_tip(MakeToolTip(),button, _("Kill all active rip and encode processes"),NULL);
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked",GTK_SIGNAL_FUNC(KillRip),(gpointer)ginfo);
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked",GTK_SIGNAL_FUNC(KillEncode),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),button,FALSE,FALSE,0);
  gtk_widget_show(button);

  button=gtk_button_new_with_label(_("Abort Ripping Only"));
  //  gtk_tooltips_set_tip(MakeToolTip(),button, _("Kill rip process"),NULL);
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked", GTK_SIGNAL_FUNC(KillRip),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),button,FALSE,FALSE,0);
  gtk_widget_show(button);

  button=gtk_button_new_with_label(_("DDJ Scan"));
  //  gtk_tooltips_set_tip(MakeToolTip(),button, _("Insert disc information into the DigitalDJ database"), NULL);
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked", GTK_SIGNAL_FUNC(DBScan),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),button,FALSE,FALSE,0);
  gtk_widget_show(button);
  ...
  //  gtk_signal_connect(GTK_OBJECT(check),"clicked", GTK_SIGNAL_FUNC(RipPartialChanged),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox2),check,FALSE,FALSE,0);
  gtk_widget_show(check);

  uinfo->partial_rip_box=gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
  gtk_widget_set_sensitive(uinfo->partial_rip_box,ginfo->rip_partial);

  hbox2=gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);

  button=gtk_button_new_with_label(_("Play"));
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked", GTK_SIGNAL_FUNC(PlaySegmentCB), (gpointer)ginfo);
  ...
  layout=gtk_widget_create_pango_layout(GTK_WIDGET(uinfo->appwin), _("Enc: Trk 99 (99.9x)"));
  ...
  uinfo->smile_indicator=NewBlankPixmap(uinfo->appwin);
  //  gtk_tooltips_set_tip(MakeToolTip(),uinfo->smile_indicator, _("Rip status"),NULL);
  ...
  for (mycpu=0; mycpu<ginfo->num_cpu; mycpu++)
    {
      ...
      gtk_box_pack_start(GTK_BOX(hbox),uinfo->mp3_prog_label[mycpu], FALSE,FALSE,0);
...
static GtkWidget *MakeRangeSelects(GripInfo *ginfo)
{
  ...
  numentry = MakeNumEntry(&(ginfo->gui_info.start_sector_entry), &ginfo->start_sector,_("Start sector"),10);
  gtk_box_pack_start(GTK_BOX(vbox),numentry,FALSE,FALSE,0);
  gtk_widget_show(numentry);
  
  numentry = MakeNumEntry(&(ginfo->gui_info.end_sector_entry), &ginfo->end_sector,_("End sector"),10);
  gtk_box_pack_start(GTK_BOX(vbox),numentry,FALSE,FALSE,0);
  gtk_widget_show(numentry);
  ...
  for(track=0;track<ginfo->disc.num_tracks;track++)
    {
      ...
      TranslateString(ginfo->mp3fileformat,str,TranslateSwitch, &enc_track,TRUE,&(ginfo->sprefs));
...
static gboolean AddM3U(GripInfo *ginfo)
{
  ...
  fp=fopen(conv_str, "w");
  if(fp==NULL) {
    grip_app_warning(ginfo->gui_info.appwin,_("Error: can't open m3u file."));
    return FALSE;
  }
  g_free(conv_str);
...
void KillRip(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->ripping) {
    ...
    for (track=0;track<ginfo->disc.num_tracks;++track)
      {
	if ((!IsDataTrack(&(ginfo->disc),track)) && (TrackIsChecked(&(ginfo->gui_info),track)))
	  {
...
void UpdateRipProgress(GripInfo *ginfo)
{
  ...
  if(ginfo->ripping) {
    ...
    if(quarter<4)
      CopyPixmap(GTK_IMAGE(uinfo->rip_pix[quarter]),
		 GTK_IMAGE(uinfo->rip_indicator));

#ifdef CDPAR
    if(ginfo->using_builtin_cdp) 
      {
	if(uinfo->minimized)
	  CopyPixmap(GTK_IMAGE(uinfo->smile_pix[ginfo->rip_smile_level]), GTK_IMAGE(uinfo->lcd_smile_indicator));
	else
	  CopyPixmap(GTK_IMAGE(uinfo->smile_pix[ginfo->rip_smile_level]), GTK_IMAGE(uinfo->smile_indicator));
      }
#endif
	...
	CopyPixmap(GTK_IMAGE(uinfo->empty_image),
		   GTK_IMAGE(uinfo->lcd_smile_indicator));
	CopyPixmap(GTK_IMAGE(uinfo->empty_image),
		   GTK_IMAGE(uinfo->smile_indicator));
      }
	...
	Debug(_("Rip partial %d  num wavs %d\n"),ginfo->rip_partial, ginfo->num_wavs);
	Debug(_("Next track is %d, total is %d\n"), NextTrackToRip(ginfo),ginfo->disc.num_tracks);

	if(!ginfo->rip_partial && (ginfo->num_wavs < ginfo->max_wavs || NextTrackToRip(ginfo) == ginfo->disc.num_tracks))
	  {
  ...
  for(mycpu=0; mycpu<ginfo->num_cpu; mycpu++)
	...
	if(stat(ginfo->mp3file[mycpu],&mystat)>=0)
	  ...
	  gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(uinfo->mp3progbar[mycpu]), percent);
	  ...
	  if(elapsed < 0.1f) /* 1/10 sec. */
	    ...
	    speed=(gfloat)mystat.st_size / ((gfloat)ginfo->kbits_per_sec * 128.0f * elapsed);
	  
	  sprintf(buf,_("Enc: Trk %d (%3.1fx)"), ginfo->mp3_enc_track[mycpu]+1,speed);
      ...
      if(quarter < 4)
        CopyPixmap(GTK_IMAGE(uinfo->mp3_pix[quarter]), GTK_IMAGE(uinfo->mp3_indicator[mycpu]));

      if (!ginfo->rip_partial && !ginfo->stop_encode &&  now!=ginfo->mp3_started[mycpu])
	{
	  ...
	  gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(uinfo->all_encprogbar), percent);
	}
      ...
      if(waitpid(ginfo->mp3pid[mycpu], NULL, WNOHANG))
	{
	  ...
	  CopyPixmap(GTK_IMAGE(uinfo->empty_image), GTK_IMAGE(uinfo->mp3_indicator[mycpu]));
	  ...
	  if(!ginfo->stop_encode)
		...
		{
		  conv_str=g_filename_to_utf8(ginfo->rip_delete_file[mycpu], strlen(ginfo->rip_delete_file[mycpu]), &rb,&wb,NULL);
		...
		ID3Add(ginfo,ginfo->mp3file[mycpu], ginfo->encoded_track[mycpu]);
		...
		TranslateAndLaunch(ginfo->mp3_filter_cmd,TranslateSwitch,
	      ...
	      if(ginfo->ripping_a_disc&&!ginfo->rip_partial && !ginfo->ripping&&ginfo->num_wavs<ginfo->max_wavs)
		{
		  if(RipNextTrack(ginfo))
		    ginfo->doencode=TRUE;
		  else
        ...
        if(!(ginfo->encoding&(1<<mycpu)))
	  {
	    gtk_label_set_text(GTK_LABEL(uinfo->mp3_prog_label[mycpu]), _("Enc: Idle"));
	  }
	}
      }  
    }
  ...
  if ((!result || ginfo->stop_encode) &&  !ginfo->encode_list && !ginfo->ripping)
    {
...
void DoRip(GtkWidget *widget, gpointer data)
{
  ...
  if(!ginfo->have_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin,
  ...
  if(NextTrackToRip(ginfo) == ginfo->disc.num_tracks)
    {
      grip_app_ok_cancel_modal(ginfo->gui_info.appwin, _("No tracks selected.\nRip whole CD?\n"),
			       G_CALLBACK(RipWholeCD), (gpointer)ginfo);
      return;
    }
...
static gboolean RipNextTrack(GripInfo *ginfo)
{
  ...
  if(ginfo->have_disc&&ginfo->rip_track>=0)
    ...
    CopyPixmap(GTK_IMAGE(uinfo->rip_pix[0]),GTK_IMAGE(uinfo->rip_indicator));
    ...
    TranslateString(ginfo->ripfileformat,str,TranslateSwitch, &enc_track,TRUE,&(ginfo->sprefs));
    ...
    if(!CanWrite(ginfo->ripfile))
      {
	grip_app_warning(ginfo->gui_info.appwin, _("No write access to write wav file"));
	return FALSE;
      }
    ...
    utf8_ripfile=g_filename_to_utf8(ginfo->ripfile,strlen(ginfo->ripfile), &rb,&wb,NULL);
    ...
    if(bytesleft<(ginfo->ripsize*1.5))
      {
	grip_app_warning(ginfo->gui_info.appwin, _("Out of space in output directory"));
	return FALSE;
      }
	...
	pthread_create(&ginfo->cdp_thread,NULL,(void *)ThreadRip, (void *)ginfo);
...
#endif
      strcpy(enc_track.wav_filename,ginfo->ripfile);
      MakeTranslatedArgs(ginfo->ripcmdline,args,100,TranslateSwitch, &enc_track,FALSE,&(ginfo->sprefs));
...
*/
      ...
      for(arg=0;args[arg];arg++)
	{
      ...
      ginfo->curr_pipe_fd = GetStatusWindowPipe(ginfo->gui_info.rip_status_window);
      ginfo->rippid = fork();
      if(ginfo->rippid == 0)
	{
	  CloseStuff(ginfo);
...
void FillInTrackInfo(GripInfo *ginfo,int track,EncodeTrack *new_track)
{
  ...
  g_snprintf(new_track->song_name,256,"%s", ginfo->ddata.data_track[track].track_name);
  g_snprintf(new_track->song_artist,256,"%s", ginfo->ddata.data_track[track].track_artist);
...
static void AddToEncode(GripInfo *ginfo, int track)
{
  ...
  Debug(_("Added track %d to %s list\n"), track+1, ginfo->delayed_encoding ? "pending" : "encoding");
}
...
static gboolean MP3Encode(GripInfo *ginfo)
{
  ...
  CopyPixmap(GTK_IMAGE(uinfo->mp3_pix[0]), GTK_IMAGE(uinfo->mp3_indicator[cpu]));
  ...
  if(!CanWrite(ginfo->mp3file[cpu]))
    {
      grip_app_warning(ginfo->gui_info.appwin, _("No write access to write encoded file."));
      return FALSE;
    }
  
  bytesleft=BytesLeftInFS(ginfo->mp3file[cpu]);
  
  conv_str=g_filename_to_utf8(ginfo->mp3file[cpu],strlen(ginfo->mp3file[cpu]), &rb,&wb,NULL);
  ...
  MakeTranslatedArgs(ginfo->mp3cmdline,args,100,TranslateSwitch, enc_track,FALSE,&(ginfo->sprefs));
...
*/
  ...
  ginfo->curr_pipe_fd = GetStatusWindowPipe(ginfo->gui_info.encode_status_window);
...
void CalculateAll(GripInfo *ginfo)
{
  ...
  for (track=0;track<ginfo->disc.num_tracks;++track)
    {
    if (!IsDataTrack(&(ginfo->disc),track) && (TrackIsChecked(uinfo,track)))
      {
  ...
  if(!ginfo->looking_up)
    {
      ...
      if(CDStat(disc,FALSE) && disc->disc_present && CDStat(disc,TRUE))
	{
	  ...
	  if(CheckTracks(disc))
	    {
	      ...
	      if(ginfo->play_first)
		if(disc->disc_mode == CDAUDIO_COMPLETED || disc->disc_mode == CDAUDIO_NOSTATUS)
		  {
		    fprintf(stderr, "%s %s %d\n", __FUNCTION__, __FILE__, __LINE__);
		    SelectRow(ginfo,0);
		    disc->curr_track = 1;
