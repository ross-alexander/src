...
void UpdateTray(GripInfo *ginfo)
{
	...
	if (ginfo->show_tray_icon) {
		...
		// gtk_tooltip_set_tip(GTK_TOOLTIP(uinfo->tray_tips), uinfo->tray_ebox, text, NULL);
...
{
  gtk_widget_set_sensitive(GTK_WIDGET(widget), (long)data);
}
...
static void MakeTrayIcon(GripInfo *ginfo)
{
	...
	img = (GtkWidget*)Loadxpm(uinfo->appwin, menuplay_xpm);
	...
	img = (GtkWidget*)Loadxpm(uinfo->appwin, menupause_xpm);
	...
	img = (GtkWidget*)Loadxpm(uinfo->appwin, menustop_xpm);
	...
	img = (GtkWidget*)Loadxpm(uinfo->appwin, rip1_xpm);
	...
	mentry = (GtkWidget*)BuildMenuItem("application-exit", _("Quit"), TRUE);
...
static gboolean TrayIconButtonPress(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	...
	if (event->button == 1) {
		if (ginfo->app_visible) {
			...
			gtk_widget_hide(GTK_WIDGET(uinfo->appwin));
		} else {
			gtk_window_move(GTK_WINDOW(uinfo->appwin), uinfo->x, uinfo->y);
			gtk_window_present(GTK_WINDOW(uinfo->appwin));
		}
