...
static char *DiscDBMakeRequest(DiscDBServer *server,DiscDBHello *hello, char *cmd)
{
  GString *uri;
  GString *proxy,*user;
  char user_agent[256];
  ...
  if(curl_handle) {
      ...
      if(*server->proxy->username)
	{
	  ...
	  g_string_sprintf(user,"%s:%s",server->proxy->username, server->proxy->pswd);
...
gboolean DiscDBDoQuery(DiscInfo *disc,DiscDBServer *server, DiscDBHello *hello,DiscDBQuery *query)
{
  int index;
  GString *cmd;
  ...
  g_string_sprintfa(cmd,"cddb+query+%08x+%d",DiscDBDiscid(disc), disc->num_tracks);
  ...
  switch(strtol(strtok(inbuffer," "), NULL,10))
    ...
    case 200:
      ...
      query->query_list[0].list_genre = DiscDBGenreValue(g_strstrip(strtok(NULL," ")));
    
      sscanf(g_strstrip(strtok(NULL," ")),"%xd", &query->query_list[0].list_id);
    
      DiscDBParseTitle(g_strstrip(strtok(NULL,"")), query->query_list[0].list_title, query->query_list[0].list_artist,"/");
      break;
...
int DiscDBReadDiscData(DiscInfo *disc,DiscData *ddata, const char *encoding)
{
  ...
  *(ddata->data_playlist)='\0';
  ...
  if(!DiscDBUTF8Validate(disc,ddata))
    {
      DiscDBConvertEncoding(disc, ddata, (char*)(strcasecmp(encoding, "UTF-8") ? encoding : "ISO-8859-1"), "UTF-8");
    }  
  fclose(discdb_data);
  return 0;
}
