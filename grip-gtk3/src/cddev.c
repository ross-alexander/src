...
gboolean CDInitDevice(char *device_name, DiscInfo *disc)
{
  ...
  if (disc->devname && disc->devname != device_name && strcmp(device_name, disc->devname))
    {
      free(disc->devname);
      disc->devname = 0;
    }
...
/* ----------------------------------------------------------------------
--
-- Update a CD status structure... because operating system interfaces vary so does this function.
--
---------------------------------------------------------------------- */
...
gboolean CDStat(DiscInfo *disc, gboolean read_toc)
{
  ...
  disc->curr_frame = (cdsc.cdsc_absaddr.msf.minute * 60 + cdsc.cdsc_absaddr.msf.second) * 75 + cdsc.cdsc_absaddr.msf.frame;

  /*
  fprintf(stderr, "%s %s %d curr_frame:%d min:%d sec:%d frame:%d\n", __FUNCTION__, __FILE__, __LINE__, disc->curr_frame,
	  cdsc.cdsc_absaddr.msf.minute,
	  cdsc.cdsc_absaddr.msf.second,
  ...
  */
  ...
  if(read_toc)
    ...
    for(readtracks=0;readtracks<=disc->num_tracks;readtracks++)
      {
	disc->track[readtracks].start_pos.mins = cdte.data[readtracks].addr.msf.minute;
	disc->track[readtracks].start_pos.secs = cdte.data[readtracks].addr.msf.second;
	...
	disc->track[readtracks].flags=(cdte.data[readtracks].addr_type << 4) | (cdte.data[readtracks].control & 0x0f);
      }
#endif
	...
	disc->track[readtracks].flags=(cdte.cdte_adr << 4) | (cdte.cdte_ctrl & 0x0f);
      }
#endif
	...
	//	fprintf(stderr, "readtracks:%d start_frame:%d\n", readtracks, disc->track[readtracks].start_frame);

	if(readtracks > 0)
	  {
	    pos=(disc->track[readtracks].start_pos.mins * 60 + disc->track[readtracks].start_pos.secs) -
	      (disc->track[readtracks-1].start_pos.mins * 60 + disc->track[readtracks -1].start_pos.secs);

	/* Compensate for the gap before a data track */
	    if((readtracks < disc->num_tracks && IsDataTrack(disc,readtracks) && (pos > 152)))
	      {
		pos-=152;
	      }
    ...
    disc->length.mins = disc->track[disc->num_tracks].start_pos.mins;
    disc->length.secs = disc->track[disc->num_tracks].start_pos.secs;
  }
   
  disc->curr_track=0;
  //  fprintf(stderr, "curr_frame: %d curr_track->start_frame:%d\n", disc->curr_frame, disc->track[disc->curr_track].start_frame);

  while((disc->curr_track < disc->num_tracks) && (disc->curr_frame >= disc->track[disc->curr_track].start_frame))
    disc->curr_track++;
  ...
  //  fprintf(stderr, "%s %s %d curr_track:%d num_tracks:%d\n", __FUNCTION__, __FILE__, __LINE__, disc->curr_track, disc->num_tracks);
...
{  
  ...
  fprintf(stderr, "Using CDIOCALLOW\n");
  
  if(ioctl(disc->cd_desc,CDIOCALLOW)<0)
    g_print(_("Unlock failed: %d"),errno);
#endif

  fprintf(stderr, "Attemping CDIOCJECT\n");
  
  if(ioctl(disc->cd_desc,CDIOCEJECT)<0)
    {
      fprintf(stderr, "CDIOCJECT failed: %s\n", strerror(errno));
...
#ifdef CDROMEJECT
  fprintf(stderr, "Attempting CDROMEJECT\n");

  if(ioctl(disc->cd_desc,CDROMEJECT)<0)
    {
      fprintf(stderr, "CDROMJECT failed: %s\n", strerror(errno));
      return FALSE;
    }
#endif
