...
static MP3Encoder encoder_defaults[]={{"bladeenc","-%b -QUIT %w %m","mp3"},
				      ...
				      {"mp3encode","-p 2 -l 3 -b %b %w %m", "mp3"},
				      {"gogo","-b %b %w %m","mp3"},
				      {"oggenc", "-o %m -a %a -l %d -t %n -b %b -N %t -G %G -d %y %w", "ogg"},
				      {"flac","-V -o %m %w","flac"},
				      {"other","",""},
				      {"",""}
};
...
gboolean LoadEncoderConfig(GripInfo *ginfo,int encodecfg)
{
  ...
  fprintf(stderr, "EncoderConfigFile: %s\n", buf);
...
void SaveEncoderConfig(GripInfo *ginfo,int encodecfg)
{
  ...
  sprintf(buf,"%s/%s-%s",getenv("HOME"),ginfo->config_filename, encoder_defaults[encodecfg].name);
  
  if(!SaveConfig(buf,"GRIP",2,encode_cfg_entries))
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Error: Unable to save encoder config."));
    }
}
...
gboolean LoadRipperConfig(GripInfo *ginfo,int ripcfg)
{
  ...
  assert(ginfo->config_filename);
  assert(ripcfg >= 0 && ripcfg < 4);

  sprintf(buf, "%s/%s-%s", getenv("HOME"),
	  ginfo->config_filename,
          ripper_defaults[ripcfg].name);

  fprintf(stderr, "RipperConfigFile: %s\n", buf);
...
void SaveRipperConfig(GripInfo *ginfo,int ripcfg)
{
  ...
  if(!SaveConfig(buf,"GRIP",2,rip_cfg_entries))
    grip_app_warning(ginfo->gui_info.appwin, _("Error: Unable to save ripper config."));
}
...
void MakeConfigPage(GripInfo *ginfo)
{
  ...
  check=MakeCheckButton(NULL,&ginfo->no_interrupt,_("Don't interrupt playback on exit/startup"));
  ...
  while(*(rip->name))
    {
      ...
      //      g_object_set_qdata(G_OBJECT(item), ripper_quark(), (gpointer)rip);
      //      gtk_signal_connect(GTK_OBJECT(item),"activate", GTK_SIGNAL_FUNC(RipperSelected),(gpointer)ginfo);
  ...
  uinfo->rip_exe_box=gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);

  entry=MakeStrEntry(&(uinfo->ripexename_entry),ginfo->ripexename, _("Ripping executable"),255,TRUE);
  gtk_box_pack_start(GTK_BOX(uinfo->rip_exe_box),entry,FALSE,FALSE,0);
  gtk_widget_show(entry);

  entry=MakeStrEntry(&(uinfo->ripcmdline_entry),ginfo->ripcmdline, _("Rip command-line"),255,TRUE);
  gtk_box_pack_start(GTK_BOX(uinfo->rip_exe_box),entry,FALSE,FALSE,0);
  gtk_widget_show(entry);
...
#ifdef CDPAR
  uinfo->rip_builtin_box=gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
  ...
  check=MakeCheckButton(NULL,&ginfo->disable_extra_paranoia, _("Disable extra paranoia"));
  gtk_box_pack_start(GTK_BOX(uinfo->rip_builtin_box),check,FALSE,FALSE,0);
  gtk_widget_show(check);
  ...
  check=MakeCheckButton(NULL,&ginfo->calc_gain, _("Calculate gain adjustment"));
  gtk_box_pack_start(GTK_BOX(uinfo->rip_builtin_box),check,FALSE,FALSE,0);
  gtk_widget_show(check);
  ...
  while(*(enc->name))
    {
      ...
      //      gtk_signal_connect(GTK_OBJECT(item),"activate", GTK_SIGNAL_FUNC(EncoderSelected),(gpointer)ginfo);
      //    gtk_menu_append(GTK_MENU(menu),item);
      //    gtk_widget_show(item);
    enc++;
  }
  ...
  dbvbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 4);
  ...
  //  gtk_signal_connect(GTK_OBJECT(button),"clicked", GTK_SIGNAL_FUNC(UseProxyChanged),(gpointer)ginfo);
  gtk_box_pack_start(GTK_BOX(vbox),check,FALSE,FALSE,0);
  gtk_widget_show(check);
...
static void RipperSelected(GtkWidget *widget,gpointer data)
{
  ...
  ginfo=(GripInfo *)data;
  uinfo=&(ginfo->gui_info);
  rip=(Ripper *)g_object_get_qdata(G_OBJECT(widget), ripper_quark());
...
G_DEFINE_QUARK(Encoder, encoder);
...
GripInfo *ginfo;
  ...
  enc=(MP3Encoder *)g_object_get_qdata(G_OBJECT(widget), encoder_quark());
