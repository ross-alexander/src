...
gboolean AppWindowStateCB(GtkWidget *widget, GdkEventWindowState *event, gpointer data)
{
  ...
  if ((state & GDK_WINDOW_STATE_WITHDRAWN) || (state & GDK_WINDOW_STATE_ICONIFIED)) {
    ...
    gtk_window_get_position(GTK_WINDOW(uinfo->appwin), &uinfo->x, &uinfo->y);
    return TRUE;
  }
		   ...
		   gboolean local_mode, gboolean no_redirect)
{
  ...
  g_object_set_qdata(G_OBJECT(appwin),g_quark_from_string("GripInfo"), (gpointer)ginfo);

  uinfo=&(ginfo->gui_info);
  uinfo->appwin = appwin;

  uinfo->status_window = 0;
  uinfo->rip_status_window = 0;
  uinfo->encode_status_window = 0;
  uinfo->track_list = 0;
  ...
  gtk_window_set_wmclass(GTK_WINDOW(appwin), "grip", "Grip");
  g_signal_connect(G_OBJECT(appwin), "delete_event", G_CALLBACK(GripDie),NULL);

  if(uinfo->minimized)
    {
      gtk_widget_set_size_request(GTK_WIDGET(appwin),MIN_WINWIDTH, MIN_WINHEIGHT);
      gtk_window_resize(GTK_WINDOW(appwin),uinfo->win_width_min, uinfo->win_height_min);
    }
  else
    {
      gtk_widget_set_size_request(GTK_WIDGET(appwin),WINWIDTH, WINHEIGHT);
      if(uinfo->track_edit_visible)
	{
	  gtk_window_resize(GTK_WINDOW(appwin),uinfo->win_width, uinfo->win_height_edit);
	}
      else
	{
	  gtk_window_resize(GTK_WINDOW(appwin),uinfo->win_width, uinfo->win_height);
	}
    }
  ...
  uinfo->winbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 3);
  if(!uinfo->minimized)
  ...
  gtk_box_pack_start(GTK_BOX(uinfo->winbox),uinfo->track_edit_box, FALSE, FALSE, 0);
  ...
  */
  ...
  gtk_container_add(GTK_CONTAINER(appwin), uinfo->winbox);
  ...
  g_signal_connect(appwin, "window-state-event", G_CALLBACK(AppWindowStateCB), ginfo);
...
void GripDie(GtkWidget *widget,gpointer data)
{
  ...
  ginfo=(GripInfo *)g_object_get_qdata(G_OBJECT(widget), g_quark_from_string("GripInfo"));
  
#ifndef GRIPCD
#if Gnome2
  if(ginfo->ripping_a_disc || ginfo->encoding)
    gnome_app_ok_cancel_modal((GnomeApp *)ginfo->gui_info.appwin,
...
#ifndef GRIPCD
  logo=Loadxpm(GTK_WIDGET(uinfo->appwin), grip_xpm);
#else
  logo=Loadxpm(GTK_WIDGET(uinfo->appwin), gcd_xpm);
#endif
...
#ifdef Gnome2
  gtk_widget_set_style(GTK_BIN(button)->child, uinfo->style_dark_grey);
#endif
  //  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(Homepage),NULL);
...
static void LoadImages(GripGUI *uinfo)
{
  uinfo->check_image=Loadxpm(uinfo->appwin,check_xpm);
  uinfo->eject_image=Loadxpm(uinfo->appwin,eject_xpm);
  uinfo->cdscan_image=Loadxpm(uinfo->appwin,cdscan_xpm);
  uinfo->ff_image=Loadxpm(uinfo->appwin,ff_xpm);
  uinfo->lowleft_image=Loadxpm(uinfo->appwin,lowleft_xpm);
  uinfo->lowright_image=Loadxpm(uinfo->appwin,lowright_xpm);
  uinfo->minmax_image=Loadxpm(uinfo->appwin,minmax_xpm);
  uinfo->nexttrk_image=Loadxpm(uinfo->appwin,nexttrk_xpm);
  uinfo->playpaus_image=Loadxpm(uinfo->appwin,playpaus_xpm);
  uinfo->prevtrk_image=Loadxpm(uinfo->appwin,prevtrk_xpm);
  uinfo->loop_image=Loadxpm(uinfo->appwin,loop_xpm);
  uinfo->noloop_image=Loadxpm(uinfo->appwin,noloop_xpm);
  uinfo->random_image=Loadxpm(uinfo->appwin,random_xpm);
  uinfo->playlist_image=Loadxpm(uinfo->appwin,playlist_xpm);
  uinfo->playnorm_image=Loadxpm(uinfo->appwin,playnorm_xpm);
  uinfo->quit_image=Loadxpm(uinfo->appwin,quit_xpm);
  uinfo->rew_image=Loadxpm(uinfo->appwin,rew_xpm);
  uinfo->stop_image=Loadxpm(uinfo->appwin,stop_xpm);
  uinfo->upleft_image=Loadxpm(uinfo->appwin,upleft_xpm);
  uinfo->upright_image=Loadxpm(uinfo->appwin,upright_xpm);
  uinfo->vol_image=Loadxpm(uinfo->appwin,vol_xpm);
  uinfo->discdbwht_image=Loadxpm(uinfo->appwin,discdbwht_xpm);
  uinfo->rotate_image=Loadxpm(uinfo->appwin,rotate_xpm);
  uinfo->edit_image=Loadxpm(uinfo->appwin,edit_xpm);
  uinfo->progtrack_image=Loadxpm(uinfo->appwin,progtrack_xpm);
  uinfo->mail_image=Loadxpm(uinfo->appwin,mail_xpm);
  uinfo->save_image=Loadxpm(uinfo->appwin,save_xpm);

  uinfo->empty_image=NewBlankPixmap(uinfo->appwin);

  uinfo->discdb_pix[0]=Loadxpm(uinfo->appwin,discdb0_xpm);
  uinfo->discdb_pix[1]=Loadxpm(uinfo->appwin,discdb1_xpm);

  uinfo->play_pix[0]=Loadxpm(uinfo->appwin,playnorm_xpm);
  uinfo->play_pix[1]=Loadxpm(uinfo->appwin,random_xpm);
  uinfo->play_pix[2]=Loadxpm(uinfo->appwin,playlist_xpm);

#ifndef GRIPCD
  uinfo->rip_pix[0]=Loadxpm(uinfo->appwin,rip0_xpm);
  uinfo->rip_pix[1]=Loadxpm(uinfo->appwin,rip1_xpm);
  uinfo->rip_pix[2]=Loadxpm(uinfo->appwin,rip2_xpm);
  uinfo->rip_pix[3]=Loadxpm(uinfo->appwin,rip3_xpm);

  uinfo->mp3_pix[0]=Loadxpm(uinfo->appwin,enc0_xpm);
  uinfo->mp3_pix[1]=Loadxpm(uinfo->appwin,enc1_xpm);
  uinfo->mp3_pix[2]=Loadxpm(uinfo->appwin,enc2_xpm);
  uinfo->mp3_pix[3]=Loadxpm(uinfo->appwin,enc3_xpm);

  uinfo->smile_pix[0]=Loadxpm(uinfo->appwin,smile1_xpm);
  uinfo->smile_pix[1]=Loadxpm(uinfo->appwin,smile2_xpm);
  uinfo->smile_pix[2]=Loadxpm(uinfo->appwin,smile3_xpm);
  uinfo->smile_pix[3]=Loadxpm(uinfo->appwin,smile4_xpm);
  uinfo->smile_pix[4]=Loadxpm(uinfo->appwin,smile5_xpm);
  uinfo->smile_pix[5]=Loadxpm(uinfo->appwin,smile6_xpm);
  uinfo->smile_pix[6]=Loadxpm(uinfo->appwin,smile7_xpm);
  uinfo->smile_pix[7]=Loadxpm(uinfo->appwin,smile8_xpm);
#endif
}
...
void GripUpdate(GtkWidget *app)
{
  ...
  ginfo=(GripInfo *)g_object_get_qdata(G_OBJECT(app), g_quark_from_string("GripInfo"));
...
{
  gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(uinfo->appwin)), uinfo->wait_cursor);
  UpdateGTK();
}

void UnBusy(GripGUI *uinfo)
{
  gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(uinfo->appwin)), NULL);
  UpdateGTK();
}
...
void DoSaveConfig(GripInfo *ginfo)
{
  ...
  if(!SaveConfig(filename,"GRIP",2,cfg_entries))
    grip_app_warning(ginfo->gui_info.appwin, _("Error: Unable to save config file."));
...
int grip_app_warning(GtkWidget *app, const char *s)
{
  fprintf(stderr, "%s\n", s);
}

int grip_app_ok_cancel_modal(GtkWidget *app, const char *s, GCallback cb, gpointer data)
{
}
