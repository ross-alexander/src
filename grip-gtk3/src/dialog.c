...
void BoolDialog(char *question,char *yes,GripSignalFunc yesfunc,
		gpointer yesdata,
		char *no,GripSignalFunc nofunc,gpointer nodata)
{
  GtkWidget *dialog;
  GtkWidget *label;
  GtkWidget *yesbutton;
  ...
  gtk_box_pack_start(GTKV_BOX(GTK_DIALOG(dialog)),label,TRUE,TRUE,0);
  ...
  gtk_signal_connect_object(GTK_OBJECT(yesbutton),"clicked",
			    ...
			    GTK_OBJECT(dialog));
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)),yesbutton,
		     TRUE,TRUE,0);
  ...
  if(no) {
    ...
    gtk_signal_connect_object(GTK_OBJECT(nobutton),"clicked",
			      GTK_SIGNAL_FUNC(gtk_widget_destroy),
			      GTK_OBJECT(dialog));
    gtk_box_pack_start(GTK_VBOX(GTK_DIALOG(dialog)),nobutton,
		       TRUE,TRUE,0);
...
void InputDialog(char *prompt,char *default_str,int len,char *doit,
		 GripSignalFunc doitfunc,
		 char *cancel,GripSignalFunc cancelfunc)
{
  GtkWidget *dialog;
  GtkWidget *label;
  ...
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)),entry,TRUE,TRUE,0);
  ...
  gtk_signal_connect_object(GTK_OBJECT(doitbutton),"clicked",
			    GTK_SIGNAL_FUNC(gtk_widget_destroy),
			    GTK_OBJECT(dialog));
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)),doitbutton,
		     TRUE,TRUE,0);
  ...
  if(cancel) {
    ...
    gtk_signal_connect_object(GTK_OBJECT(cancelbutton),"clicked",
			      GTK_SIGNAL_FUNC(gtk_widget_destroy),
			      GTK_OBJECT(dialog));
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)),cancelbutton,
		       TRUE,TRUE,0);
...
GtkWidget *MakeStrEntry(GtkWidget **entry,char *var,char *name, int len,gboolean editable)
{
  GtkWidget *widget;
  GtkWidget *label;
  GtkWidget *hbox;
  ...
  widget=gtk_entry_new(); // _with_max_length(len);
  gtk_editable_set_editable(GTK_EDITABLE(widget),editable);
  if(var)
    {
      gtk_entry_set_text(GTK_ENTRY(widget),var);
      g_signal_connect(G_OBJECT(widget),"changed", G_CALLBACK(ChangeStrVal),(gpointer)var);
    }
...
GtkWidget *MakeNumEntry(GtkWidget **entry, int *var, char *name, int len)
{
  ...
  widget=gtk_entry_new(); // _with_max_length(len);
  gtk_widget_set_size_request(widget,len*8+5,0);

  if(var)
    {
      sprintf(buf,"%d",*var);
      gtk_entry_set_text(GTK_ENTRY(widget),buf);
      g_signal_connect(G_OBJECT(widget),"changed", G_CALLBACK(ChangeIntVal),(gpointer)var);
    }
  ...
  if(var)
    {
      ...
      g_signal_connect(G_OBJECT(widget),"clicked", G_CALLBACK(ChangeBoolVal), (gpointer)var);
    }
