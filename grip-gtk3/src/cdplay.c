...
static gboolean TracklistButtonPressed(GtkWidget *widget,GdkEventButton *event, gpointer data);
void SelectRow(GripInfo *ginfo,int track);
...
void InitProgram(GripInfo *ginfo);
static void ShuffleTracks(GripInfo *ginfo);
gboolean CheckTracks(DiscInfo *disc);
...
static void ShutDownCB(GtkWidget *widget,gpointer data)
{
  ...
  GripDie(ginfo->gui_info.appwin,NULL);
}
...
static void DiscDBToggle(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->looking_up)
    ...
    if(ginfo->ripping_a_disc)
      {
	grip_app_warning(ginfo->gui_info.appwin, _("Cannot do lookup while ripping."));
	return;
      }
    if(ginfo->have_disc)
  ...
  if(track_list)
    {
      ...
      tot_width+=width;
      layout = gtk_widget_create_pango_layout(GTK_WIDGET(track_list), _("Rip"));
      pango_layout_get_size(layout,&width,NULL);
      g_object_unref(layout);
      tot_width+=width;
  ...
  if(track_list)
    {
      ...
      column = gtk_tree_view_get_column(GTK_TREE_VIEW(track_list), TRACKLIST_TRACK_COL);
      int width = gtk_tree_view_column_get_width(column);
      gtk_tree_view_column_set_fixed_width(column, width - tot_width);
    }
}
...
void MakeTrackPage(GripInfo *ginfo)
{
  ...
  /* Add vbox to put elements into */
  
  vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
  gtk_container_set_border_width(GTK_CONTAINER(vbox), 3);

  /* Add static disc and artist name at top of the box */
  ...
  uinfo->track_list = gtk_tree_view_new_with_model(GTK_TREE_MODEL(uinfo->track_list_store));
  ...
  gtk_tree_view_column_set_fixed_width(column, uinfo->win_width - (GetLengthRipWidth(ginfo)+15));
  ...
  g_signal_connect(G_OBJECT(column),"clicked", G_CALLBACK(ClickColumn),(gpointer)ginfo);
  gtk_tree_view_append_column(GTK_TREE_VIEW(uinfo->track_list), column);
  select = gtk_tree_view_get_selection(GTK_TREE_VIEW(uinfo->track_list));
  gtk_tree_selection_set_mode(select, GTK_SELECTION_SINGLE);
  g_signal_connect(G_OBJECT(select),"changed", G_CALLBACK(SelectionChanged),(gpointer)ginfo);
  g_signal_connect(G_OBJECT(uinfo->track_list),"button_press_event", G_CALLBACK(TracklistButtonPressed), (gpointer)ginfo);

  /* g_signal_connect(G_OBJECT(uinfo->track_list),"cursor_changed", G_CALLBACK(SelectRow), (gpointer)ginfo); */
  /* g_signal_connect(G_OBJECT(uinfo->track_list),"unselect_row", G_CALLBACK(UnSelectRow), (gpointer)uinfo); */
  /* g_signal_connect(G_OBJECT(uinfo->track_list),"button_press_event", G_CALLBACK(CListButtonPressed),(gpointer)uinfo); */
  /* g_signal_connect(G_OBJECT(uinfo->track_list),"click_column",  G_CALLBACK(ClickColumn),(gpointer)ginfo); */

  /* Create scrolled window object and put track selection into it */
  ...
  gtk_widget_get_preferred_size(uinfo->track_list, 0, &sizereq);
  gtk_widget_set_size_request(trackpage,sizereq.width+30,-1);
  gtk_widget_set_size_request(trackpage,500,-1);
...
void SetCurrentTrackIndex(GripInfo *ginfo, int track)
{
  ...
  for(ginfo->current_track_index = 0; (ginfo->current_track_index < MAX_TRACKS) && (ginfo->current_track_index < ginfo->prog_totaltracks) && (CURRENT_TRACK != track); ginfo->current_track_index++)
    continue;
}
  ...
  if(track < 0)
    {
      ...
      g_signal_handlers_block_by_func(G_OBJECT(uinfo->track_edit_entry), TrackEditChanged,(gpointer)ginfo);
      gtk_entry_set_text(GTK_ENTRY(uinfo->track_edit_entry),"");
    
      g_signal_handlers_unblock_by_func(G_OBJECT(uinfo->track_edit_entry), TrackEditChanged,(gpointer)ginfo);
      g_signal_handlers_block_by_func(G_OBJECT(uinfo->track_artist_edit_entry), TrackEditChanged,(gpointer)ginfo);
    
      gtk_entry_set_text(GTK_ENTRY(uinfo->track_artist_edit_entry),"");
      g_signal_handlers_unblock_by_func(G_OBJECT(uinfo->track_artist_edit_entry), TrackEditChanged,(gpointer)ginfo);
    }
  else
    {
      //      fprintf(stderr, "* %s\n", ginfo->ddata.data_track[track].track_name);
      g_signal_handlers_block_by_func(G_OBJECT(uinfo->track_edit_entry), TrackEditChanged,(gpointer)ginfo);
      gtk_entry_set_text(GTK_ENTRY(uinfo->track_edit_entry), ginfo->ddata.data_track[track].track_name);
      g_signal_handlers_unblock_by_func(G_OBJECT(uinfo->track_edit_entry), TrackEditChanged,(gpointer)ginfo);

      //      fprintf(stderr, "* %s\n", ginfo->ddata.data_track[track].track_artist);

      g_signal_handlers_block_by_func(G_OBJECT(uinfo->track_artist_edit_entry),TrackEditChanged,(gpointer)ginfo);
      gtk_entry_set_text(GTK_ENTRY(uinfo->track_artist_edit_entry), ginfo->ddata.data_track[track].track_artist);
      g_signal_handlers_unblock_by_func(G_OBJECT(uinfo->track_artist_edit_entry),TrackEditChanged,(gpointer)ginfo);
      ...
      tracklen=(ginfo->disc.track[track+1].start_frame-1) - ginfo->disc.track[track].start_frame;
...
gboolean TrackIsChecked(GripGUI *uinfo,int track)
{
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(uinfo->track_list_store), &iter,TRACKLIST_RIP_COL,&checked,-1);
...
void SetChecked(GripGUI *uinfo,int track,gboolean checked)
{
  ...
  path = gtk_tree_path_new_from_indices(track,-1);
  gtk_tree_model_get_iter(GTK_TREE_MODEL(uinfo->track_list_store), &iter,path);
  gtk_list_store_set(uinfo->track_list_store,&iter, TRACKLIST_RIP_COL, checked, -1);
...
static gboolean TracklistButtonPressed(GtkWidget *widget,GdkEventButton *event, gpointer data)
{
  ...
  if(gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(uinfo->track_list), event->x,event->y, &path,&column,NULL,NULL))
    {
      indices=gtk_tree_path_get_indices(path);
      row=indices[0];
      cols=gtk_tree_view_get_columns(GTK_TREE_VIEW(uinfo->track_list));
...
void SelectRow(GripInfo *ginfo, int track)
{
  ...
  //  fprintf(stderr, "selectrow 1 -- track %d\n", track);
  path = gtk_tree_path_new_from_indices(track, -1);
  assert(path);
  select = gtk_tree_view_get_selection(GTK_TREE_VIEW(ginfo->gui_info.track_list));
...
static void SelectionChanged(GtkTreeSelection *selection, gpointer data)
{
  ...
  if((ginfo->disc.disc_mode == CDAUDIO_PLAYING) && (ginfo->disc.curr_track!=(row+1)))
    PlayTrack(ginfo,row);
}
...
static void PlaylistChanged(GtkWindow *window, GtkWidget *widget, gpointer data)
{
  ...
  strcpy(ginfo->ddata.data_playlist, gtk_entry_get_text(GTK_ENTRY(ginfo->gui_info.playlist_entry)));
...
static void ToggleLoop(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->playloop) 
    CopyPixmap(GTK_IMAGE(ginfo->gui_info.loop_image), GTK_IMAGE(ginfo->gui_info.loop_indicator));
  else
    CopyPixmap(GTK_IMAGE(ginfo->gui_info.noloop_image), GTK_IMAGE(ginfo->gui_info.loop_indicator));
}
...
static void ChangePlayMode(GtkWidget *widget,gpointer data)
{
  ...
  CopyPixmap(GTK_IMAGE(ginfo->gui_info.play_pix[ginfo->play_mode]), GTK_IMAGE(ginfo->gui_info.play_indicator));
  gtk_widget_set_sensitive(GTK_WIDGET(ginfo->gui_info.playlist_entry), ginfo->play_mode==PM_PLAYLIST);
...
GtkWidget *MakePlayOpts(GripInfo *ginfo)
{
  ...
  g_signal_connect(G_OBJECT(uinfo->playlist_entry),"focus_out_event", G_CALLBACK(PlaylistChanged),(gpointer)ginfo);
  gtk_widget_set_sensitive(GTK_WIDGET(uinfo->playlist_entry), ginfo->play_mode==PM_PLAYLIST);
  gtk_box_pack_start(GTK_BOX(hbox),uinfo->playlist_entry,TRUE,TRUE,0);
  gtk_widget_show(uinfo->playlist_entry);

  uinfo->play_indicator=NewBlankPixmap(uinfo->appwin);
  CopyPixmap(GTK_IMAGE(uinfo->play_pix[ginfo->play_mode]), GTK_IMAGE(uinfo->play_indicator));
  ...
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(ChangePlayMode),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Rotate play mode"));
  gtk_widget_show(button);

  uinfo->loop_indicator=NewBlankPixmap(uinfo->appwin);

  if(ginfo->playloop)
    CopyPixmap(GTK_IMAGE(uinfo->loop_image), GTK_IMAGE(uinfo->loop_indicator));
  else
    CopyPixmap(GTK_IMAGE(uinfo->noloop_image), GTK_IMAGE(uinfo->loop_indicator));
  ...
  // gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(ToggleLoop),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Toggle loop play"));
  gtk_widget_show(button);
...
GtkWidget *MakeControls(GripInfo *ginfo)
{
  ...
  g_signal_connect(G_OBJECT(lcdbox),"button_press_event", G_CALLBACK(ToggleControlButtons),(gpointer)ginfo);
  ...
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(ChangeTimeMode),(gpointer)ginfo);
  ...
  indicator_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_box_set_homogeneous(GTK_BOX(indicator_box), TRUE);

  uinfo->rip_indicator=NewBlankPixmap(GTK_WIDGET(uinfo->appwin));
  gtk_box_pack_start(GTK_BOX(indicator_box),uinfo->rip_indicator,TRUE,TRUE,0);
  gtk_widget_show(uinfo->rip_indicator);

  uinfo->lcd_smile_indicator=NewBlankPixmap(GTK_WIDGET(uinfo->appwin));
  gtk_widget_set_tooltip_text(uinfo->lcd_smile_indicator,_("Rip status"));
  gtk_box_pack_start(GTK_BOX(indicator_box),uinfo->lcd_smile_indicator, TRUE,TRUE,0);
  gtk_widget_show(uinfo->lcd_smile_indicator);

  for(mycpu=0;mycpu<ginfo->num_cpu;mycpu++)
    {
      uinfo->mp3_indicator[mycpu]=NewBlankPixmap(GTK_WIDGET(uinfo->appwin));
      gtk_box_pack_start(GTK_BOX(indicator_box), uinfo->mp3_indicator[mycpu],TRUE,TRUE,0);
      gtk_widget_show(uinfo->mp3_indicator[mycpu]);
    }
  
  uinfo->discdb_indicator=NewBlankPixmap(GTK_WIDGET(uinfo->appwin));
  gtk_box_pack_start(GTK_BOX(indicator_box), uinfo->discdb_indicator, TRUE,TRUE,0);
  ...
  g_signal_connect(adj,"value_changed", G_CALLBACK(SetVolume),(gpointer)ginfo);
  uinfo->volume_control=gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, GTK_ADJUSTMENT(adj));
  ...
  uinfo->control_button_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
  gtk_box_set_homogeneous(GTK_BOX(uinfo->control_button_box), TRUE);

  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_box_set_homogeneous(GTK_BOX(hbox), TRUE);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->playpaus_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(PlayTrackCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Play track / Pause play"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->rew_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"pressed", G_CALLBACK(RewindCB),(gpointer)ginfo);
  g_signal_connect(G_OBJECT(button),"released", G_CALLBACK(RewindCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button,_("Rewind"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->ff_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"pressed", G_CALLBACK(FastFwdCB),(gpointer)ginfo);
  g_signal_connect(G_OBJECT(button),"released", G_CALLBACK(FastFwdCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("FastForward"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->prevtrk_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(PrevTrackCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Go to previous track"));
  gtk_widget_show(button);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->nexttrk_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(NextTrackCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Go to next track"));
  gtk_widget_show(button);

  button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->progtrack_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(ToggleProg),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Toggle play mode options"));
  gtk_widget_show(button);

  if(ginfo->changer_slots>1)
    {
      button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->rotate_image);
    //    gtk_widget_set_style(button,uinfo->style_dark_grey);
      gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
      g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(NextDisc),(gpointer)ginfo);
      gtk_widget_set_tooltip_text(button, _("Next Disc"));
      gtk_widget_show(button);
    }
  ...
  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->stop_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(StopPlayCB),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Stop play"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->eject_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(EjectDisc),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Eject disc"));
  gtk_widget_show(button);
  ...
  //  gtk_widget_set_tooltip_text(button, _("Scan Disc Contents"));
  //  gtk_widget_show(button);

  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->vol_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(ToggleVol),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button, _("Toggle Volume Control"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->edit_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  // g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(ToggleTrackEdit),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button,_("Toggle disc editor"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  if(!ginfo->local_mode)
    {
      button=ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->discdbwht_image);
      //    gtk_widget_set_style(button,uinfo->style_dark_grey);
      g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(DiscDBToggle),(gpointer)ginfo);
      gtk_widget_set_tooltip_text(button, _("Initiate/abort DiscDB lookup"));
      gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
      gtk_widget_show(button);
    }

  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->minmax_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  g_signal_connect(G_OBJECT(button),"clicked", G_CALLBACK(MinMax),(gpointer)ginfo);
  gtk_widget_set_tooltip_text(button,_("Toggle track display"));
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  gtk_widget_show(button);

  button = ImageButton(GTK_WIDGET(uinfo->appwin),uinfo->quit_image);
  //  gtk_widget_set_style(button,uinfo->style_dark_grey);
  gtk_widget_set_tooltip_text(button,_("Exit Grip"));
  
  gtk_box_pack_start(GTK_BOX(hbox),button,TRUE,TRUE,0);
  g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(ShutDownCB),(gpointer)ginfo);
  gtk_widget_show(button);
  ...
  if(uinfo->minimized)
    {
      ...
      gtk_box_set_child_packing(GTK_BOX(uinfo->winbox), uinfo->controls,FALSE,FALSE,0,GTK_PACK_START);
      gtk_widget_show(uinfo->notebook);

      CopyPixmap(GTK_IMAGE(uinfo->lcd_smile_indicator), GTK_IMAGE(uinfo->smile_indicator));
      CopyPixmap(GTK_IMAGE(uinfo->empty_image), GTK_IMAGE(uinfo->lcd_smile_indicator));
      
      gtk_widget_set_size_request(GTK_WIDGET(uinfo->appwin), WINWIDTH, WINHEIGHT);
  ...
  else
    {
      ...
      gtk_box_set_child_packing(GTK_BOX(uinfo->winbox),uinfo->controls, TRUE,TRUE,0,GTK_PACK_START);

      gtk_widget_hide(uinfo->notebook);

      CopyPixmap(GTK_IMAGE(uinfo->smile_indicator), GTK_IMAGE(uinfo->lcd_smile_indicator));
    ...
    // if(uinfo->track_prog_visible) ToggleProg(NULL,(gpointer)ginfo);
      ...
      gtk_widget_set_size_request(GTK_WIDGET(uinfo->appwin), MIN_WINWIDTH,MIN_WINHEIGHT);
      gtk_window_resize(GTK_WINDOW(uinfo->appwin), uinfo->win_width_min, uinfo->win_height_min);
...
static void ToggleControlButtons(GtkWidget *widget,GdkEventButton *event, gpointer data)
{
  GripGUI *uinfo;
...
static void FastFwdCB(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot fast forward while ripping."));
      return;
    }
...
void FastFwd(GripInfo *ginfo)
{
  ...
  if((ginfo->disc.disc_mode==CDAUDIO_PLAYING) || (ginfo->disc.disc_mode==CDAUDIO_PAUSED))
    {
...
static void RewindCB(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot rewind while ripping."));
      return;
    }
...
static void NextDisc(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot switch discs while ripping."));
      return;
    }
...
void EjectDisc(GtkWidget *widget, gpointer data)
{
  ...
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot eject while ripping."));
      return;
    }
  ...
  if(ginfo->have_disc)
    {
      ...
      printf("EjectDisc -- have_disc %d %d\n", ginfo->have_disc);
    ...
    // if(!ginfo->tray_open) CheckNewDisc(ginfo,FALSE);
  }
...
void PlayTrackCB(GtkWidget *widget,gpointer data)
{
  ...
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot play while ripping."));
      return;
    }
...
void NextTrack(GripInfo *ginfo)
{
  if(ginfo->ripping_a_disc)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Cannot switch tracks while ripping."));
      return;
    }
...
void InitProgram(GripInfo *ginfo)
{
  ...
  if(mode==PM_PLAYLIST)
    {
      plist = strdup(gtk_entry_get_text(GTK_ENTRY(ginfo->gui_info.playlist_entry)));
...
gboolean CheckTracks(DiscInfo *disc)
{
  ...
  for(track=0;track<disc->num_tracks;track++)
    if(disc->track[track].length.mins || disc->track[track].length.secs)
      have_track=TRUE;
...
void ScanDisc(GtkWidget *widget,gpointer data)
{
  ...
  if (NULL == p_cdio)
    ...
    cdtext_select_language(cdtext, CDTEXT_LANGUAGE_ENGLISH);
    unsigned int i_track = 0;
    for (int i = 0; i < MAX_CDTEXT_FIELDS; i++)
      {
        if (cdtext_get_const(cdtext, i, i_track))
	  {
	    if (strcmp(cdtext_field2str(i), "TITLE") == 0)
	      SetTitle(ginfo, (char*)cdtext_get_const(cdtext, i, i_track));

	    if (strcmp(cdtext_field2str(i), "PERFORMER") == 0)
	      SetArtist(ginfo, cdtext_get_const(cdtext, i, i_track));
	  }
      }

    for (int i_track = i_first_track ; i_track < i_last_track; i_track++)
      {
	...
	for (int i = 0; i < MAX_CDTEXT_FIELDS; i++)
	  {
	    if (cdtext_get_const(cdtext, i, i_track))
	      {
		if (strcmp(cdtext_field2str(i), "TITLE") == 0)
		  strcpy(ginfo->ddata.data_track[i_track - i_first_track].track_name, cdtext_get_const(cdtext, i, i_track));
...
{
  /* Note: need another solution other than statics if we ever want to be reentrant */
  ...
  if(!uinfo->minimized)
	...
	gtk_window_get_size(GTK_WINDOW(uinfo->appwin), &uinfo->win_width, &uinfo->win_height_edit);
      else
	gtk_window_get_size(GTK_WINDOW(uinfo->appwin), &uinfo->win_width, &uinfo->win_height);

      //      fprintf(stderr, "%s %s %d\n", __FUNCTION__, __FILE__, __LINE__);
      
      gtk_widget_get_allocation(uinfo->track_list, &alloc);
      
      if(old_width && (old_width != alloc.width))
	ResizeTrackList(ginfo);
      old_width = alloc.width;
    }
  else
    {
      gtk_window_get_size(GTK_WINDOW(uinfo->appwin),&uinfo->win_width_min, &uinfo->win_height_min);
    }
  ...
  if(!ginfo->looking_up)
    ...
    {
      CopyPixmap(GTK_IMAGE(uinfo->discdb_pix[discdb_counter++%2]), GTK_IMAGE(uinfo->discdb_indicator));
    }
  ...
  if(ginfo->update_required)
    {
      ...
      if(ginfo->have_disc)
	{
	  g_snprintf(buf,80,"%02d:%02d",disc->length.mins, disc->length.secs);
	  g_snprintf(icon_buf, sizeof(icon_buf),"%02d %s %s", disc->curr_track,buf,PACKAGE);
	  
	  gtk_label_set_text(GTK_LABEL(uinfo->play_time_label),buf);
	  
	  if(!ginfo->looking_up)
	    {
	      CopyPixmap(GTK_IMAGE(uinfo->empty_image), GTK_IMAGE(uinfo->discdb_indicator));
      ...
      else
	{
      ...
      gdk_window_set_icon_name(gtk_widget_get_window(uinfo->appwin),icon_buf);
    }
}
...
//  fprintf(stderr, "%s %s %d\n", __FUNCTION__, __FILE__, __LINE__);
  ...
  if(ginfo->have_disc)
    {
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(uinfo->multi_artist_button), ginfo->ddata.data_multi_artist);
	...
	//	TranslateAndLaunch(ginfo->cdupdate,TranslateSwitch,&enc_track, FALSE,&(ginfo->sprefs),CloseStuff,(void *)ginfo);
	}
    }
  else
    {
  ...
  gtk_entry_set_text(GTK_ENTRY(uinfo->playlist_entry), ddata->data_playlist);
...
//    fprintf(stderr, "%s %d -- current_track %d\n", __FUNCTION__, __LINE__, CURRENT_TRACK);
  ...
  if(ginfo->ask_submit)
    {
      grip_app_ok_cancel_modal(uinfo->appwin,
...
void SubmitEntry(gint reply,gpointer data)
{
  ...
  if(fd == -1)
    {
      grip_app_warning(ginfo->gui_info.appwin, _("Error: Unable to create temporary file."));
      return;
    }
  ...
  if(!efp)
	...
	grip_app_warning(ginfo->gui_info.appwin, _("Error: Unable to write disc data."));
	fclose(efp);
      }
      else
	{
