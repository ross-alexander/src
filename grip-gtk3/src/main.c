...
{
  //  GnomeClient *client = calloc(sizeof(GnomeClient), 1);
  ...
  //  gchar *app_id = "net.hepazulian.grip";
  //  assert(g_application_id_is_valid(app_id));
  //  client->app = gtk_application_new(app_id, 0);
  //  assert(client->app);

#ifdef Gnome2  
  client = gnome_master_client();
  gtk_signal_connect(GTK_OBJECT(client),"save_yourself", GTK_SIGNAL_FUNC(SaveSession),argv[0]);
  gtk_signal_connect(GTK_OBJECT(client),"die", GTK_SIGNAL_FUNC(KillSession),NULL);
#endif
  ...
  g_timeout_add(1000, TimeOut, grip_app);
  gtk_main();
  return 0;
}
