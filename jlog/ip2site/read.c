...
static char *med3(char *a, char *b, char *c, int (*cmp) (void*, void*))
{   return cmp(a, b) < 0 ?
       (cmp(b, c) < 0 ? b : cmp(a, c) < 0 ? c : a)
     : (cmp(b, c) > 0 ? b : cmp(a, c) > 0 ? c : a);
}

void quicksort(char *a, size_t n, size_t es, int (*cmp) (void*, void*))
{
...
struct baserec {
   ...
   nexthop_t nexthop; /* pointer to next-hop table */
};
...
struct prerec {
   ...
   nexthop_t nexthop; /* pointer to nexthop table */
};
                           ...
                           int verbose)
{
   ...
   for (i = 0; i < size; i++)
	...
	printf("++ %lu %d %lu %d\n", entry[i]->data, entry[i]->len, entry[i+1]->data, entry[i+1]->len);
